...
view_selection(DataCosts const & data_costs, UniGraph * graph, Segmentation const & segmentation, Settings const &) {
    ...
    const size_t num_segments = 1; // TODO
    const size_t NO_SEGMENT = num_segments;

    // to node ids
    size_t node_id_counter = num_segments; // reserve first nodes for segment placeholders
    std::vector<std::size_t> node_ids(num_faces);
    std::vector<std::vector<std::size_t > > faces_of_segments(num_segments + 1);
    for(size_t i = 0; i < segmentation.size(); ++i) {
        if (segmentation[i] == 4 || segmentation[i] == 5) {
            node_ids[i] = 0; // the eye segment
            faces_of_segments[0].push_back(i);
        }
        else {
            node_ids[i] = node_id_counter++;
            faces_of_segments[NO_SEGMENT].push_back(i);
        }
    }
    ...
    // accumulate segments
    std::vector<DataCosts::Column> segment_costs(num_segments);
    for (size_t segment_id = 0; segment_id < num_segments; ++segment_id)
    {
        ...
        for (size_t i = 0; i < summed_costs.size(); ++i) {
            if (summed_costs[i].first == faces_of_segments[segment_id].size()) {
                ...
                segment_costs[segment_id].push_back(label_cost_pair);
            }
        }
    }
    ...
    for (std::size_t i = 0; i < num_faces; ++i) {
        ...
        std::vector<UniGraph::WeightedEdge> adj_faces = graph->get_adj_nodes(i);
        for (auto const & adj_edge : adj_faces) {
            std::size_t adj_face = adj_edge.first;
            if (data_costs.col(adj_face).empty()) continue;

            /* Uni directional */
            if (adj_face < i && node_ids[adj_face] != node_id) {
                mgraph.increase_edge_weight(node_ids[adj_face], node_id, adj_edge.second);
            }
        }
    }
